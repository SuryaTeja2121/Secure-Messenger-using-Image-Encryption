...
def encrypt(key, source, encode=True):
    ...
    IV = Random.new().read(DES3.block_size)  # generate IV
    encryptor = DES3.new(key, DES3.MODE_EAX, IV)
    # calculate needed padding
    padding = DES3.block_size - len(source) % DES3.block_size
...
def decrypt(key, source, decode=True):
    ...
    # use SHA-256 over our key to get a proper-sized AES key
    key = MD5.new(key).digest()
    IV = source[:DES3.block_size]  # extract the IV from the beginning
    decryptor = DES3.new(key, DES3.MODE_EAX, IV)
    data = decryptor.decrypt(source[DES3.block_size:])  # decrypt
            ...
            for ch in message:
                ...
                for i in range(0, 8):
                    ...
                    if current_bit == '0':
                        if three_pixels[i] % 2 != 0:
                            three_pixels[i] = three_pixels[i] - \
                                1 if three_pixels[i] == 255 else three_pixels[i]+1
                    elif current_bit == '1':
                        if three_pixels[i] % 2 == 0:
                            three_pixels[i] = three_pixels[i] - \
                                1 if three_pixels[i] == 255 else three_pixels[i]+1
                ...
                if(tmp == len(message)):
                    ...
                    if three_pixels[-1] % 2 == 0:
                        three_pixels[-1] = three_pixels[-1] - \
                            1 if three_pixels[-1] == 255 else three_pixels[-1]+1
                else:
                ...
                for i in range(0, 3):
                    if DEBUG:
                        print("Putting pixel at ", (x, y),
                              " to ", three_pixels[st:end])
...
def print_credits():
    ...
    table.add_column("Registration Number", style="yellow")
    table.add_row("Charan Tej", "BL.EN.U4AIE19013")
    table.add_row("Surya Teja Chavali", "BL.EN.U4AIE19014")
    table.add_row("Sugash T.M.", "BL.EN.U4AIE19062")
    console.print(table)
...
if __name__ == "__main__":
    ...
    print()
    print()
    print("Our Project allows you to hide texts inside an image. You can also protect these messages with a password using AES-256 algorithm.\n")
    main()